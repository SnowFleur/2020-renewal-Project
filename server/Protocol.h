...
- 현재 View에서 멀어지면 Send 안하게 하는거 까지 확인했음
- 문제는 이제 시야에서 멀어져서 더 이상 패킷을 보내지 않지만 클라이언트에서 그대로 멈추는 문제가 있음
- 이 문제가 발생하는 원인은 섹터 크기만큼 Near리스트에 유저가 있기 때문임(Remove 안함)
- 해결 방법은 현재 oldView를 처리하는 방법을 바꾸던지(view와 거리 차이 계산해서 빼던지)
- view거리만큼 가지고 있는거랑 해서 ?
